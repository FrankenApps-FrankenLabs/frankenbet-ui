...
export default function App() {
  ...
  const fetchBalances = useCallback(async (ct, address) => {
    try {
      const hb = await ct.houseBalance();
      const pb = await ct.getPlayerBalance(address);
      setHouseBalance(parseFloat(formatEther(hb)).toFixed(2));
  ...
  const dealGame = () => {
    ...
    setPlayerCards(p);
    setDealerCards(d);
    setCurrentBet(betNum);
    setGameState(GAME_STATE.PLAYING);
    setSessionBalance(prev => (parseFloat(prev) - betNum).toFixed(2));
    if (handTotal(p) === 21) {
      finishHands(p, null, d, remaining, betNum, betNum);
    }
  };
    ...
    if (activeHand === 0) {
      ...
      if (handTotal(newHand) >= 21) {
        ...
        if (splitCards !== null) {
          ...
          finishHands(newHand, null, dealerCards, remaining, currentBet, currentBet);
        }
      }
    } else {
      const newSplit = [...splitCards, newCard];
      setSplitCards(newSplit);
      if (handTotal(newSplit) >= 21) {
        finishHands(playerCards, newSplit, dealerCards, remaining, currentBet, currentBet);
      }
    }
  };

  const doStand = () => {
    if (activeHand === 0 && splitCards !== null) {
      setActiveHand(1);
      setGameState(GAME_STATE.SPLIT_PLAYING);
    } else {
      finishHands(playerCards, splitCards, dealerCards, deck, currentBet, currentBet);
    }
  };
  ...
  const doDoubleDown = () => {
    ...
    if (activeHand === 0) {
      ...
      if (splitCards !== null) {
        ...
        finishHands(newHand, null, dealerCards, remaining, newBet, newBet);
      }
    } else {
      const newSplit = [...splitCards, newCard];
      setSplitCards(newSplit);
    ...
    <div style={S.app}>
      <style>{`
        ...
        @keyframes glitch {
          ...
          50%{transform:translate(0,2px)} 75%{transform:translate(0,-2px) skew(0.5deg)}
      ...
      <div style={{ textAlign: 'center', marginBottom: '1rem', position: 'relative', zIndex: 1 }}>
        ...
        {walletAddress && (
          <div style={{ display: 'flex', gap: '1.5rem', justifyContent: 'center', marginTop: '0.5rem', flexWrap: 'wrap' }}>
            <div style={{ color: '#ff00ff', fontSize: '0.75rem', letterSpacing: '2px', textShadow: '0 0 8px #ff00ff' }}>HOUSE: {houseBalance} LCAI</div>
            <div style={{ color: '#00ffcc', fontSize: '0.75rem', letterSpacing: '2px', textShadow: '0 0 8px #00ffcc' }}>SESSION: {sessionBalance} LCAI</div>
          </div>
        )}
      </div>
        ...
        <div style={S.resultOverlay} onClick={() => setResults(null)}>
          {results.map((r, i) => (
            <div key={i} style={{ textAlign: 'center' }}>
              {results.length > 1 && (
                <div style={{ color: '#666', fontSize: '0.7rem', letterSpacing: '3px', marginBottom: '0.25rem' }}>
                  {i === 0 ? 'HAND 1' : 'HAND 2'}
                </div>
              )}
              <div style={{ fontSize: results.length > 1 ? 'clamp(1.5rem, 5vw, 3rem)' : 'clamp(2rem, 8vw, 5rem)', fontWeight: 900, color: r.color, textShadow: `0 0 20px ${r.color}, 0 0 60px ${r.color}`, letterSpacing: '6px', animation: glitch ? 'glitch 0.1s infinite' : 'none' }}>
                {r.label}
              </div>
        ...
        <>
          ...
          {screen === 'deposit' && (
            <div style={S.section}>
              ...
              <div style={{ color: '#666', fontSize: '0.75rem', letterSpacing: '2px', marginBottom: '1rem', lineHeight: '1.6' }}>
                Deposit once — play unlimited hands with no MetaMask pop-ups. Cash out anytime.
              </div>
          ...
          {screen === 'game' && (
            <>
                ...
                <>
                    ...
                    <div style={S.section}>
                      ...
                      <div style={S.cardRow}>
                        {dealerCards.map((card, i) => {
                          ...
                          return (
                            <div key={i} style={{ ...S.card, color: card.isRed ? '#cc0000' : '#111' }}>
                              <div>{card.value}</div>
                  ...
                  {playerCards.length > 0 && (
                    <div style={activeHand === 0 && isPlaying ? S.activeSection : S.section}>
                      <div style={S.sectionLabel}>
                        ◈ {splitCards ? 'Hand 1' : 'You'} — {handTotal(playerCards)}
                        {activeHand === 0 && isPlaying && <span style={{ color: '#ffd700', marginLeft: '0.5rem' }}>◄ ACTIVE</span>}
                      </div>
                      <div style={S.cardRow}>
                  ...
                  {splitCards && (
                    <div style={activeHand === 1 && isPlaying ? S.activeSection : S.section}>
                      <div style={S.sectionLabel}>
                        ◈ Hand 2 — {handTotal(splitCards)}
                        {activeHand === 1 && isPlaying && <span style={{ color: '#ffd700', marginLeft: '0.5rem' }}>◄ ACTIVE</span>}
                      </div>
                      <div style={S.cardRow}>
              ...
              {status && (
                <div style={{ color: '#00ffcc', fontSize: '0.8rem', letterSpacing: '2px', textAlign: 'center', textShadow: '0 0 8px #00ffcc', zIndex: 1, marginBottom: '0.5rem' }}>
                  ⚡ {status}
                </div>
              )}
            </>
